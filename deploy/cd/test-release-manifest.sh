...
baseline_tuple_sha="sha256:3333333333333333333333333333333333333333333333333333333333333333"
...
if node deploy/cd/release-manifest.mjs create \
  ...
  --backend-image "ghcr.io/cheese-work/multica-backend@$backend_digest" \
  --web-image "ghcr.io/cheese-work/multica-web@$web_digest" \
  --output "$tmp_dir/no-tuple.json" >/dev/null 2>&1; then
  echo "release candidate without a baseline tuple was accepted" >&2
  exit 1
fi
