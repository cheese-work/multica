...
type EnforcedTimeouts struct {
	...
	// AttemptLockKey, when nonzero, is acquired via pg_advisory_lock_shared
	// on every connection this pool opens (see below). Zero disables the
	// attempt-marking behavior entirely, preserving exact prior behavior
	// for every caller that does not set it.
	AttemptLockKey int64
}
...
//
// When enforced.AttemptLockKey is nonzero, the same AfterConnect hook also
// acquires a SESSION-LEVEL SHARED advisory lock at that key on every
// connection (main migration connection and every later hook connection
// alike) and never releases it — it is released automatically when each
// connection closes. This is CHE-372's attempt-bound ownership marker
// (see deploy/cd/quiescence.mjs's verifyLiveSessionHoldsAttemptLock): the
// key is a fresh, cryptographically random per-attempt secret generated by
// the CD supervisor and never logged, persisted, or included in any
// connection string, so a foreign client — which by definition does not
// know this run's secret — cannot acquire the same key and therefore
// cannot be mistaken for a session this attempt actually opened. Shared
// mode (not exclusive) is required because the migrator legitimately opens
// more than one connection (its pinned migration connection plus
// hook connections), and all of them must be able to hold this marker
// concurrently; shared-mode advisory locks permit any number of
// simultaneous holders, so this does not serialize or block those
...
func NewPoolWithEnforcedTimeouts(ctx context.Context, databaseURL string, connectTimeout time.Duration, enforced EnforcedTimeouts) (*pgxpool.Pool, error) {
	...
	attemptLockKey := enforced.AttemptLockKey
	cfg.AfterConnect = func(ctx context.Context, conn *pgx.Conn) error {
		...
		if attemptLockKey != 0 {
			var granted bool
			if err := conn.QueryRow(ctx, "SELECT pg_try_advisory_lock_shared($1)", attemptLockKey).Scan(&granted); err != nil {
				return fmt.Errorf("acquire attempt-marker advisory lock: %w", err)
			}
			if !granted {
				// pg_try_advisory_lock_shared only fails to grant when an
				// EXCLUSIVE holder already exists for this exact key. Since
				// this key is a per-attempt secret nobody else knows, that
				// can only mean key reuse (a bug, not a foreign client) —
				// still reject rather than proceed with an unmarked
				// connection that would silently fail the fencing check.
				return fmt.Errorf("attempt-marker advisory lock (key %d) was not granted in shared mode — an exclusive holder already exists for this key", attemptLockKey)
			}
		}
		return nil
	}
