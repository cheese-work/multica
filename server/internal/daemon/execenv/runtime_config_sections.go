...
func writeWorkflowIssue(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("- You delivered what the issue itself asks for and it awaits acceptance → `in_review`. On an issue with no parent of its own `done` stays human, unless your Agent Identity or Squad Operating Protocol explicitly grants you a `done` path of its own — follow that grant's exact condition when it does.\n")
	b.WriteString("- **A sub-issue closes itself.** Any issue that has a parent is a child here, including one that dispatched sub-issues of its own — having children does not exempt it. A child that met its acceptance criteria and that you verified takes a terminal status — `done`, or the applicable review verdict — rather than `in_review`. Stage barriers and parent notifications fire only when every child in the stage is terminal, so a child parked at `in_review` holds its stage open and its parent is never woken. Humans read children for breakdown, size and progress, not as a review queue; the agent side owns the child's review loop end to end, which means an independent reviewer other than the author, not the author reviewing its own work. A child escalates only when a blocker needs a person the agents cannot route around — `blocked`, with the obstacle named.\n")
	...
	if ctx.IsSquadLeader {
		b.WriteString("- Squad leader: dispatching members is not delivery — a dispatch turn leaves the parent `in_progress`, and it moves to `in_review` only on the later turn (a member update or stage-barrier re-trigger) where you confirm the overall goal is met. If your Squad Operating Protocol grants you status ownership of this parent, that protocol's `done`-vs-`in_review` choice governs the confirming turn instead of the default `in_review`-only routing above.\n")
	}
