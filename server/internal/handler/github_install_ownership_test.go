...
// TestIsGitHubConfigured_RequiresEveryConnectCredential pins that the Connect
// button fails closed for every member of the six-key prerequisite set.
func TestIsGitHubConfigured_RequiresEveryConnectCredential(t *testing.T) {
	...
	if !isGitHubConfigured() {
		t.Fatal("integration reported not configured with every credential set")
	}
	...
	for key, value := range credentials {
		t.Run(key, func(t *testing.T) {
			t.Setenv(key, "")
			if isGitHubConfigured() {
				t.Errorf("integration reported configured without %s", key)
			}
			t.Setenv(key, value)
		})
	}
