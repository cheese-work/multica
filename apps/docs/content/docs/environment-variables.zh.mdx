...
| GitHub | `GITHUB_APP_CLIENT_ID` | App 的 Client ID；用于交换安装回调中的 OAuth code 并确认连接用户 |
| GitHub | `GITHUB_APP_CLIENT_SECRET` | 与 Client ID 配对的 client secret，用途同上 |
| GitHub | `GITHUB_APP_ID` | 验证新 installation、显示 PR 卡片的 CI 状态与可合并性，以及“从 GitHub 选择”仓库都需要它 |
