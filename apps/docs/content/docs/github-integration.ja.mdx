...
Connect と既存 installation の claim を有効にするには 6 つすべてが必要です。署名済み webhook 処理は webhook secret によって独立して制御されます。
...
`GITHUB_APP_ID` と `GITHUB_APP_PRIVATE_KEY` は installation identity、リポジトリ参照、CI 状態、マージ可否のために App として認証します。秘密鍵は GitHub App の **Private keys → Generate a private key** から生成します。webhook 専用デプロイでは省略できますが、Connect は 6 つの値すべてがなければ fail-closed です。
...
- **接続ボタンを使用できない**: 6 つすべてが API プロセスに設定されていることを確認します: `GITHUB_APP_SLUG`、`GITHUB_WEBHOOK_SECRET`、`GITHUB_APP_CLIENT_ID`、`GITHUB_APP_CLIENT_SECRET`、`GITHUB_APP_ID`、`GITHUB_APP_PRIVATE_KEY`。
