...
All six are required for the Connect and existing-installation claim actions to enable; signed webhook processing remains independently gated by the webhook secret.
...
`GITHUB_APP_ID` and `GITHUB_APP_PRIVATE_KEY` authenticate the App for installation identity, repository browsing, CI status, and mergeability. Generate the private key under the GitHub App's **Private keys → Generate a private key**. They are optional only for webhook-only deployments; Connect is deliberately fail-closed without all six values.
...
- **Connect button disabled**: check that all six GitHub variables reached the API process: `GITHUB_APP_SLUG`, `GITHUB_WEBHOOK_SECRET`, `GITHUB_APP_CLIENT_ID`, `GITHUB_APP_CLIENT_SECRET`, `GITHUB_APP_ID`, and `GITHUB_APP_PRIVATE_KEY`.
