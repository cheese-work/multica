...
| GitHub | `GITHUB_APP_CLIENT_ID` | App の Client ID。インストールコールバックの OAuth code を交換し、接続ユーザーを確認するために必須です |
| GitHub | `GITHUB_APP_CLIENT_SECRET` | Client ID と対になる client secret。用途は同じです |
| GitHub | `GITHUB_APP_ID` | 新規 installation の検証、PR カードの CI ステータス・マージ可否、「GitHub から選択」リポジトリに必要です |
