...
| GitHub | `GITHUB_APP_CLIENT_ID` | App Client ID. Required to exchange the installation callback's OAuth code and identify the connecting user |
| GitHub | `GITHUB_APP_CLIENT_SECRET` | Client secret paired with the Client ID; same use |
| GitHub | `GITHUB_APP_ID` | Required to verify a new installation, show CI and mergeability on PR cards, and use the "pick from GitHub" repository picker |
