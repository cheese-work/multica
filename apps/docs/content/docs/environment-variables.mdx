...
| GitHub | `GITHUB_APP_ID` | Required for Connect, installation identity, CI status, mergeability, and the "pick from GitHub" repository picker |
| GitHub | `GITHUB_APP_PRIVATE_KEY` | Full PEM private key paired with the App ID; same uses as above. Preserve its newlines |
