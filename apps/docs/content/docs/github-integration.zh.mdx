...
Connect 和已有 installation claim 需要六个变量全部配置；签名 webhook 的处理仍由 webhook secret 独立控制。
...
`GITHUB_APP_ID` 和 `GITHUB_APP_PRIVATE_KEY` 用于以 App 身份验证 installation identity、仓库浏览、CI 状态和可合并性。私钥在 GitHub App 的 **Private keys → Generate a private key** 中生成。仅 webhook 的部署可以省略它们，但 Connect 缺少六个值中的任一个都会 fail-closed。
...
- **连接按钮不可用**：检查六个变量是否都已进入 API 进程：`GITHUB_APP_SLUG`、`GITHUB_WEBHOOK_SECRET`、`GITHUB_APP_CLIENT_ID`、`GITHUB_APP_CLIENT_SECRET`、`GITHUB_APP_ID`、`GITHUB_APP_PRIVATE_KEY`。
